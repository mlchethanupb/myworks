...
X1 = dataset1.iloc[3:8, -3].values.astype(float)
#X1=X1.cumsum() 
#influxdb_memstats_heap_inuse
y1 = dataset1.iloc[3:8, -2].values.astype(float)


dataset2 = pd.read_csv("p1.csv")

X2 = dataset1.iloc[3:8, -3].values.astype(float)

y2 = dataset1.iloc[3:8, -1].values.astype(float)
...
plt.ylabel('Influxdb Disk Usage')
plt.legend()
plt.show()
