...
class Env(gym.Env):
    ...
    def get_reward(self):
        reward = 0
        if self.pa.objective != self.pa.A2C_Ctime and self.pa.objective != self.pa.PPO2_Ctime:
        ...
        elif self.pa.objective == self.pa.A2C_Ctime or self.pa.objective == self.pa.PPO2_Ctime:
            reward = 0
            remaining_jobs = 0
            for j in self.machine.running_job:
                remaining_jobs += 1
        ...
        for a in range(len(act)):
            ...
            #actions.remove(a)
                ...
                if status == 'MoveOn':  
                    ...
                    if moveOncount == self.pa.num_nw - 1:
                       break
