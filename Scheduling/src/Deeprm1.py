...
class Env(gym.Env):
    ...
    def step(self, a):
        ...
        for act in range(len(a)):
            if (a[act] == 1 or a.all() == 0):
                if a.all() == 0 or act == self.pa.job_wait_queue or self.job_slot.slot[act] is None:
                    status = 'MoveOn'
                else:
    ...
    def get_new_job_from_seq(self, seq_no, seq_idx):
        new_job = Job(resorce_requirement=self.job_sequence_size[seq_idx], job_len=self.job_sequence_len[seq_idx],
    ...
    def allocate_job(self, job, curr_time):
        ...
        for t in range(0, self.time_horizon - job.len):
            new_avbl_res = self.available_res_slot[t: t +
                                                   job.len, :] - job.resorce_requirement
