...
def plot_lr_curve(output_file_prefix, max_rew_lr_curve, mean_rew_lr_curve, slow_down_lr_curve,
    ...
    for k in ref_discount_rews:
        ax.plot(np.tile(np.average(ref_discount_rews[k]), len(mean_rew_lr_curve)), linewidth=2, label=k , color = colors[i])
        i += 1
    ax.plot(max_rew_lr_curve, linewidth=2, label='PG max' , color = 'peru')
    ...
    i = 0
    ax.plot(slow_down_lr_curve, linewidth=2, label='PG mean' , color = 'peru')
    for k in ref_discount_rews:
        ax.plot(np.tile(np.average(np.concatenate(ref_slow_down[k])), len(slow_down_lr_curve)), linewidth=2, label=k, color = colors[i])
        i += 1
    plt.legend(loc=1)
    plt.xlabel("Iteration", fontsize=20)
    plt.ylabel("Slowdown", fontsize=20)

    plt.savefig(output_file_prefix + "_lr_curve" + ".png")
