...
if __name__ == '__main__':
    ...
    #         mean_slowdown.append(np.mean(value_sl[val]))
    #         mean_ctime.append(np.mean(value_ct[val]))
    #     mean_slowdown_plot.append(np.mean(mean_slowdown))
    #     deviation.append(np.std(mean_slowdown))
    # title = ['DeepRM', 'SJF', 'Packer', 'Random']
    # colors = ['r', 'g', 'c', 'm']
    # y_pos = np.arange(len(title))
    # plt.bar(title, mean_slowdown_plot, color=colors, yerr=deviation)
    # plt.xticks(y_pos, title)
    # for i, v in enumerate(mean_slowdown_plot):
    #     plt.text(v + 3, i + .25, str(v), color='blue', fontweight='bold')

    # plt.xlabel('Performance metrics')
    # plt.ylabel('Average job slowdown')
    # plt.title('Performance for objective Minimize Average Job SLowdown')
    # plt.tight_layout()
    ...
    for i in range(len(test_types)):
        ...
        # mean_values = (np.mean(mean_slowdown), np.mean(mean_slowdown))
        # deviation = (np.std(mean_slowdown), np.std(mean_slowdown))
        mean_values = (np.mean(mean_slowdown))
        deviation = (np.std(mean_slowdown))
        agent_plot = plt.bar(index + i * bar_width, mean_values, bar_width,
                             yerr=deviation,
                             ecolor='k',
                             capsize=14,
    ...
    for agent_plot in agent_plots:
        autolabel(agent_plot, ax)
    plt.xlabel('Objective - Avg Job Slowdown')
    plt.ylabel('Average job slowdown')
    plt.title('Performance for objective mimize Average Job Slowdown')
    plt.xticks(index + 4 * bar_width, ('Average job slowdown',
                                       'Average job completion time'))
    plt.legend()
    # ax2 = ax.twinx()
    ...
    plt.tight_layout()
    fig.savefig("Performance" + str(datetime.today()) +
                str(datetime.time(datetime.now()))+".png")
