...
class Env:
    def __init__(self, pa, nw_len_seqs=None, nw_size_seqs=None,
        ...
        if nw_len_seqs is None or nw_size_seqs is None:
            ...
            print("Mean Load is :", np.mean(self.workload))
...
#orignial code
        ...
        if self.pa.objective_disc == self.pa.objective_slowdown:
           ...
           # print("The reward for this timestep is -> ",reward)
       
        # Ignore the below section at the moment
       
        elif self.pa.objective_disc == self.pa.objective_deadline:
            reward = 0
            job_missing_deadline = 0
            for j in self.machine.running_job:
                if j.job_completion_time > j.job_deadline:
                    job_missing_deadline += 1
            reward = self.pa.delay_penalty * abs(remaining_jobs)
        return reward
...
if __name__ == '__main__':
    ...
    job_sequence_len, job_sequence_size = job_distribution.generate_sequence_work(
        pa, 42)
    print("The job rate is 0.635", pa.new_job_rate)
    print('job_sequence_len -->', job_sequence_len)
    env = Env(pa, nw_len_seqs=job_sequence_len, nw_size_seqs=job_sequence_size, render=False,
              repre='image')
    ...
    # test_types = ['SJF']
    for test_type in range(len(test_types)):
        print("The current execution: ", test_types[test_type])
        ob = env.reset()
        done = False
        info = {}
        while not done:
            if test_types[test_type] == 'SJF':
                action = other_agents.get_sjf_action(env.machine, env.job_slot)
            elif test_types[test_type] == 'Packer':
                action = other_agents.get_packer_action(
                    env.machine, env.job_slot)
            ob, reward, done, info = env.step(action)
            # if env.curr_time > 199:
            #     print("")
            if done:
                ...
                jobs_completion_time[test_type].append(
    ...
    for i in range(len(test_types)):
        ...
        print('jobs_slow_down[i] -->', jobs_slow_down[i])
        mean_values = (np.mean(jobs_slow_down[i]), np.mean(
            jobs_completion_time[i]))
        deviation = (np.std(jobs_slow_down[i]),
                     np.std(jobs_completion_time[i]))
