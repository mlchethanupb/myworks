...
class MAC_RL(gym.Env):
    ...
    def fetch_nh_from_RTable(self, node, dest):
        ...
        if (self.dest in dest_list_to_search):
            ...
            # print("nxt_hop_to_transmit", self.nh_to_transmit)
        ...
        for nodes in self.graph.nodes:
            ...
            if(action_of_node == 1):
                ...
                if(len(queue)):
                    ...
                    # print("domain_list of node", domain_list)

                    for domain in domain_list:
                        # print("domains", domain)
                        collision_domain_elems = self.collision_domain_elems[domain]
                        # print("collision_domain_elems", collision_domain_elems)
                        domain_key = domain
                        # print("domain_key", domain_key)

                        num_nodes_transmitting = 0
                        for elems in collision_domain_elems:
                            if elems != nodes:
                                elems_action = []
                                index_of_elems = self.__get_index(elems)
                                elems_action.append(actions[index_of_elems])
                                for ele_action in elems_action:
                                    if ele_action == 1 and elems not in self.attack_nodes:
                                        num_nodes_transmitting += 1
                                        break

                    # print("num_nodes_transmitting", num_nodes_transmitting)

                    if num_nodes_transmitting >= 1:
                        self.packet_lost += 1
                        # print("self.packet_lost", self.packet_lost)
                        reward = self.COLLISION_REWARD
                        # print("collision reward", reward)
                        logging.debug("collision occurred")
                        self.collision_occured = True
                        break
                    elif (self.__hidden_terminal_problem(actions, nodes, domain_key, qs_list, packet_to_send)):
                        reward = self.COLLISION_REWARD
                        self.packet_lost += 1
                        ...
                        if(next_hop_for_source == destination_for_source):
                            # print("successfull transmission to destination")
                            logging.debug(
                            ...
                            self.queues[next_hop_for_source].insert(
                                0, packet_to_send)
                            # print("successfull transmission to next hop")
                            logging.debug(
                                "Successfull transmission to nexthop")
                            self.succ_transmission += 1
                # else:
                #     print("action taken on empty queue")
                #     logging.debug("action taken on empty queue")
    ...
    def __hidden_terminal_problem(self, actions, source, domain_key, qs_list, packet_to_send):
                        ...
                        if (h_key == domain_of_nh):
                            ...
                            for itr in range(len(h_action)):
                                if(h_action[itr] == 1):
                                    ...
                                    if len(other_queue):
                                        if(source != h_values[itr] and other_queue[len(other_queue)-1].nxt_hop == src_nxt_hop):
                                            num_nodes_transmitting += 1
