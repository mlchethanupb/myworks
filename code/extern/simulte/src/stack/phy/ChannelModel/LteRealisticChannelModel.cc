...
void LteRealisticChannelModel::initialize()
{
    ...
    transmitPowerUeLinear_=par("transmitPowerUeLinear");
...
// Computation of PHY layer metrics - RSSI for UL, DL and SL

std::vector<std::pair<Band,double>> LteRealisticChannelModel::getRSSI(LteAirFrame *frame, UserControlInfo* lteInfo_1, MacNodeId transmittingNodeId, Coord transmittingNodeCoord, MacNodeId enbId)
{
    rssiVectorsBands.clear();
    // some variables which will be continuously updated
    double transmitPowerUedBm = 10*log10(transmitPowerUeLinear_*1000);// dBm
    ...
    //Coorinates of source transmiter (can be UE, eNB etc)
    MacNodeId  receivingNodeId = getAncestorPar("macNodeId");
    inet::Coord receivingNodeCoord = phy_->getCoord();
    ...
    else
    {
        ...
        channelNoiseBW = -174 + 10*log10(10e7);
        // use the jakes map in the UE side
        cqiDl = true;
    }
    // Compute speed
    speed = computeSpeed(receivingNodeId, receivingNodeCoord);
    EV<<"speed: "<<speed<<endl;

    EV << "LteRealisticChannelModel::getRSSI - receiving UE=" << receivingNodeId
            << " - transmitting UE=" << transmittingNodeId
            << " - DIR=" << dirToA(dir)
            << " - frameType=" << ((lteInfo_1->getFrameType()==FEEDBACKPKT)?"feedback":"other")
            << endl
            << " - txPwr " << transmitPowerUedBm
            << " - receivingUe_Coord[" << receivingNodeCoord << "] - transmittingUe_Coord[" << transmittingNodeCoord<< "] - receiver_Id[" << receivingNodeId<< "] - transmitter_Id[" << transmittingNodeId << "]" <<
            endl;
    ...
    EV << "\t using parameters - noiseFigure=" << noiseFigure << " - antennaGainTx=" << antennaGainTx << " - antennaGainRx=" << antennaGainRx <<
            " - txPwr=" << transmitPowerUedBm << " - for ueId=" << receivingNodeId << endl;
    EV<<"Fading enabled: "<<fading_<< ""<< "Fading type: "<<fadingType_ <<endl;
    // attenuation for the desired signal
    double attenuation = getAttenuation_D2D(receivingNodeId, dir, receivingNodeCoord, transmittingNodeId, transmittingNodeCoord); // dB

    //compute attenuation (PATHLOSS + SHADOWING)
    transmitPowerUedBm -= attenuation; // (dBm-dB)=dBm

    //add antenna gain
    transmitPowerUedBm += antennaGainTx; // (dBm+dB)=dBm
    transmitPowerUedBm += antennaGainRx; // (dBm+dB)=dBm

    //sub cable loss
    transmitPowerUedBm -= cableLoss_; // (dBm-dB)=dBm
    ...
    for (unsigned int i = 0; i < band_; i++)
    {
        ...
        //if fading is enabled - transmitted signal undergoes fading
        if (fading_)
        {
            //Appling fading
            if (fadingType_ == RAYLEIGH)
                fadingAttenuation = rayleighFading(transmittingNodeId, i);

            else if (fadingType_ == JAKES)
            {
                fadingAttenuation = jakesFading(transmittingNodeId, speed, i, cqiDl);
            }

        }
        // add fading contribution to the received pwr
        double finalRecvPower = transmitPowerUedBm + fadingAttenuation; // (dBm+dB)=dBm
        ...
        EV << " LteRealisticChannelModel::getSINR_d2d node " << receivingNodeId
                << ((lteInfo_1->getFrameType() == FEEDBACKPKT) ?
                        " FEEDBACK PACKET " : " NORMAL PACKET ")
                        << " band " << i << " recvPower " << finalRecvPower
    ...
    {
        computeInCellD2DInterference(enbId, receivingNodeId, receivingNodeCoord, transmittingNodeId, transmittingNodeCoord, (lteInfo_1->getFrameType() == FEEDBACKPKT), &inCellInterference,dir);
    }
    ...
    if( enableD2DInCellInterference_ && dir==D2D  )
    {
        ...
        double den;
        EV << "LteRealisticChannelModel::getSINR - distance from my Peer = " << transmittingNodeCoord.distance(receivingNodeCoord) << " - DIR=" << dirToA(dir)  << endl;
        ...
        for ( int i = 0; i < band_; i++)
        {
            ...
            EV << "\t ext[" << extCellInterference << "] - in[" << inCellInterference[i] << "] - recvPwr["
                    << dBmToLinear(rssiVector[i]) << "] - sinr[" << rssiVector[i]-den -channelNoiseBW<< "]\n";
    ...
    else
    {
        for (int i = 0; i < band_; i++)
        {
            ...
            //rssiVector[i] = rssiVector[i] ;
            rssiVectorsBands.emplace_back(i,rssiVector[i]);
            EV<<"RSSI bands: "<< rssiVector[i]<<endl;
            EV << "LteRealisticChannelModel::getRSSI - distance from my Peer = " << transmittingNodeCoord.distance(receivingNodeCoord) << " - DIR=" << dirToA(dir) << " - rssi[" << rssiVector[i] << "]\n";
        }
    }

    //sender is a UE
    updatePositionHistory(receivingNodeId, receivingNodeCoord);



    return rssiVectorsBands;
}
...
double LteRealisticChannelModel::getAttenuation_D2D(MacNodeId nodeId, Direction dir, Coord coord,MacNodeId node2_Id, Coord coord_2)
{
    ...
    speed = computeSpeed(nodeId, coord);
    EV<<"Correlation distance: "<<correlationDistance_<<coord<<coord_2<<endl;
    //If traveled distance is greater than correlation distance UE could have changed its state and
    // its visibility from eNodeb, hence it is correct to recompute the los probability
    if (movement > correlationDistance_
            || losMap_.find(nodeId) == losMap_.end())
...
std::vector<double> LteRealisticChannelModel::getRSRP_D2D(UserControlInfo* lteInfo,std::vector<std::pair<Band,double>> rssiVector)
{
    rsrpVector.clear();
    //measured at the receiver UE
    EV<<"Computing Sidelink RSRP at the receiver UE"<<rsrpVector.size()<<endl;

    // Get Tx power
    double recvPower = lteInfo->getD2dTxPower(); // dBm
    double rsrp=0.0;
    // Coordinate the of current UE
    MacNodeId  receivingNodeId = getAncestorPar("macNodeId");
    inet::Coord receivingNodeCoord = phy_->getCoord();

    EV<<"Receiving node Id: "<<receivingNodeId <<"Coordinates of receiving node: "<<receivingNodeCoord <<endl;
    int sidelinkRBs = lteInfo->getTotalGrantedBlocks();
    EV<<"Allocated resource blocks: "<<sidelinkRBs<<endl;
    ...
    int p=0;
        ...
        for (auto iter:rssiVector)
        {
            rsrp=iter.second - 10 * log10(12*4);
